...
class PeopleStore: PeopleService {
    
    static let shared = PeopleStore()
    private init() {}
    
    func fetchPeople(from endpoint: PeopleEndPoint, page: String = "1", completion: @escaping (Result<PeopleResponse, ErrorResponse>) -> ()) {
        guard let url = URL(string: "\(Constants.baseAPIURL)/\(endpoint.rawValue)") else {
    ...
    func fetchPerson(id: Int, completion: @escaping (Result<People, ErrorResponse>) -> ()) {
        guard let url = URL(string: "\(Constants.baseAPIURL)/person/\(id)") else {
            completion(.failure(.invalidEndpoint))
            return
        }
        self.loadURLAndDecode(url: url,params: [
